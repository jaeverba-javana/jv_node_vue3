...
function getH1Component(element) {
    return defineComponent({
        name: 'H1Component',
        setUp(props, _ref2) {
            return () => {
                _createVNode(element.target)
            }
        }
    });
}
...
v-main.d-flex.fd-column.align-center
    v-container.v-main__content.fill-width.text-h5( :class="{'v-media-movile': !mobile}" )
        router-view(v-if="$route.params.postId" :postElements="postElements" nombre="Javier")
        router-view(v-else)

    div(style="height: 50px; width: 50px; background-color: rgb(var(--v-theme-primary))")
        p {{$vuetify.theme.global.current.colors.primary}}
    div(style="height: 50px; width: 50px; background-color: rgb(var(--v-theme-primary-darken-1))")
        p {{$vuetify.theme.global.current.colors["primary-darken-1"]}}
