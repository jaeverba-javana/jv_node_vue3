import os
import re
import requests

from multiprocessing import Process, Queue




icons = [
"house",
"magnifying-glass",
"user",
"check",
"download",
"image",
"phone",
"bars",
"envelope",
"star",
"location-dot",
"music",
"wand-magic-sparkles",
"face-awesome",
"heart",
"arrow-right",
"circle-xmark",
"bomb",
"poo",
"camera-retro",
"xmark",
"cloud",
"comment",
"caret-up",
"truck-fast",
"pen-nib",
"arrow-up",
"hippo",
"face-smile",
"calendar-days",
"paperclip",
"shield-halved",
"file",
"bell",
"cart-shopping",
"clipboard",
"filter",
"circle-info",
"arrow-up-from-bracket",
"alarm-clock",
"bolt",
"raygun",
"car",
"ghost",
"mug-hot",
"circle-user",
"rocket-launch",
"pen",
"umbrella",
"gift",
"film",
"grid-2",
"list",
"gear",
"trash",
"circle-up",
"circle-down",
"inbox",
"rotate-right",
"sparkles",
"lock",
"headphones",
"volume",
"barcode",
"tag",
"book",
"bookmark",
"print",
"camera",
"font",
"video",
"circle-half-stroke",
"droplet",
"pen-to-square",
"share-from-square",
"plus",
"minus",
"share",
"circle-exclamation",
"fire",
"eye",
"eye-slash",
"plane",
"magnet",
"hand",
"folder",
"folder-open",
"money-bill",
"thumbs-up",
"thumbs-down",
"comments",
"grill-hot",
"lemon",
"key",
"thumbtack",
"gears",
"paper-plane",
"code",
"globe",
"text",
"arrow-down-to-line",
"truck",
"city",
"ticket",
"tree",
"wifi",
"window",
"burger-soda",
"camera-movie",
"paint-roller",
"bicycle",
"sliders",
"brush",
"hashtag",
"megaphone",
"flask",
"briefcase",
"compass",
"dumpster-fire",
"person",
"person-dress",
"balloons",
"address-book",
"bath",
"handshake",
"snowflake",
"right-to-bracket",
"earth-americas",
"cloud-arrow-up",
"binoculars",
"palette",
"layer-group",
"brackets-curly",
"users",
"gamepad",
"business-time",
"feather",
"sun",
"computer-classic",
"link",
"pen-fancy",
"badge-check",
"fish",
"joystick",
"play-pause",
"bug",
"shop",
"mug-saucer",
"planet-ringed",
"landmark",
"campfire",
"poo-storm",
"chart-simple",
"shirt",
"files",
"dolphin",
"anchor",
"quote-left",
"atom-simple",
"bag-shopping",
"bed-front",
"people-simple",
"gauge",
"signal-bars",
"code-compare",
"user-secret",
"stethoscope",
"car-side",
"hand-holding-heart",
"ufo",
"alien-8bit",
"block-question",
"aperture",
"bird",
"strawberry",
"envelopes",
"face-vomit",
"fort",
"truck-front",
"crab",
"user-bounty-hunter",
"thought-bubble",
"cable-car",
"mountain-sun",
"traffic-cone",
"diagram-venn",
"bowl-chopsticks-noodles",
"moped",
"burrito",
"robot-astromech",
"bat",
"location-pin",
"info",
"user-minus",
"calendar",
"cart-plus",
"clock",
"person-sign",
"circle",
"play",
"cross",
"backward",
"chart-scatter",
"handshake-slash",
"chevron-up",
"passport",
"question",
"pencil",
"phone-volume",
"car-building",
"head-side",
"upload",
"strikethrough",
"computer-speaker",
"credit-card",
"street-view",
"database",
"copy",
"mobile",
"square",
"sort",
"forward",
"hourglass-start",
"signal-slash",
"newspaper",
"notes-medical",
"table",
"building",
"globe-stand",
"square-heart",
"stop",
"tally",
"store",
"flag",
"lightbulb-slash",
"file-excel",
"browser",
"network-wired",
"cash-register",
"file-export",
"hand-point-up",
"angle-up",
"shield",
"border-top",
"address-card",
"expand",
"flag-checkered",
"quote-right",
"tags",
"server",
"user-nurse",
"video-slash",
"arrow-down",
"blog",
"school",
"file-invoice",
"rocket",
"coin",
"spinner",
"square-question",
"tty",
"exclamation",
"water",
"shield-check",
"registered",
"empty-set",
"signature",
"laptop",
"phone-office",
"line-height",
"transporter-3",
"projector",
"moon-stars",
"restroom",
"power-off",
"sitemap",
"icons",
"desktop",
"moon",
"basketball-hoop",
"loveseat",
"square-info",
"calendar-week",
"pause",
"file-word",
"triangle",
"watch",
"vials",
"language",
"door-open",
"brain",
"hotel",
"marker",
"star-of-life",
"leaf",
"walkie-talkie",
"person-dolly-empty",
"shower",
"glass",
"mailbox",
"caret-down",
"light-switch",
"badge",
"file-import",
"typewriter",
"place-of-worship",
"flower",
"spinner-third",
"wallet",
"slash",
"computer-mouse-scrollwheel",
"award",
"toggle-on",
"ship",
"file-check",
"stocking",
"chalkboard",
"planet-moon",
"lights-holiday",
"hands",
"signal",
"radar",
"motorcycle",
"booth-curtain",
"arrow-up-right-from-square",
"audio-description",
"seedling",
"closed-captioning",
"text-size",
"train",
"arrow-left",
"books",
"wrench",
"microchip",
"record-vinyl",
"trophy",
"hammer",
"diamond",
"robot",
"file-pdf",
"tablet-rugged",
"hospital",
"file-contract",
"square-xmark",
"square-check",
"stars",
"crown",
"speaker",
"ballot-check",
"speakers",
"user-plus",
"mug-tea",
"solar-system",
"virus",
"child",
"scanner-touchscreen",
"repeat",
"cube",
"copyright",
"medal",
"bullseye",
"mask",
"meat",
"circle-check",
"radio",
"reply",
"album",
"chair",
"route",
"plug",
"calculator",
"dragon",
"certificate",
"fingerprint",
"road",
"crosshairs",
"heading",
"percent",
"user-tie",
"square-minus",
"i-cursor",
"church",
"joint",
"container-storage",
"comments-dollar",
"truck-monster",
"recycle",
"warehouse",
"ruler",
"font-case",
"police-box",
"soap",
"starship-freighter",
"bullseye-pointer",
"blanket",
"scroll",
"coins",
"house-night",
"alarm-snooze",
"wind",
"baby",
"lightbulb",
"file-exclamation",
"transporter-empty",
"cauldron",
"voicemail",
"puzzle-piece",
"keyboard",
"clone",
"eraser",
"wine-bottle",
"dice",
"file-spreadsheet",
"receipt",
"ring",
"lamp",
"wagon-covered",
"unlock",
"solar-panel",
"ruler-vertical",
"circle-notch",
"people-arrows",
"flower-tulip",
"draw-square",
"engine-warning",
"raindrops",
"rectangle-wide",
"dollar-sign",
"tablet",
"not-equal",
"glasses",
"headset",
"code-branch",
"gopuram",
"images",
"window-restore",
"industry",
"staff",
"stamp",
"microphone-slash",
"align-slash",
"cookie-bite",
"otter",
"chevron-down",
"kiwi-bird",
"viruses",
"kidneys",
"users-medical",
"umbrella-beach",
"microwave",
"subscript",
"tablets",
"knife-kitchen",
"game-console-handheld",
"transporter",
"microphone",
"blinds-raised",
"border-none",
"dumbbell",
"plane-departure",
"z",
"yin-yang",
"yen-sign",
"y",
"xmarks-lines",
"xmark-to-slot",
"xmark-large",
"x-ray",
"x",
"wrench-simple",
"wreath-laurel",
"wreath",
"worm",
"won-sign",
"wine-glass-empty",
"wine-glass-crack",
"wine-glass",
"windsock",
"window-minimize",
"window-maximize",
"window-frame-open",
"window-frame",
"window-flip",
"wind-warning",
"wind-turbine",
"wifi-weak",
"wifi-slash",
"wifi-fair",
"wifi-exclamation",
"whistle",
"whiskey-glass-ice",
"whiskey-glass",
"wheelchair-move",
"wheelchair",
"wheat-slash",
"wheat-awn-slash",
"wheat-awn-circle-exclamation",
"wheat-awn",
"wheat",
"whale",
"weight-scale",
"weight-hanging",
"webhook",
"waveform-lines",
"waveform",
"wave-triangle",
"wave-square",
"wave-sine",
"wave-pulse",
"watermelon-slice",
"water-ladder",
"water-arrow-up",
"water-arrow-down",
"watch-smart",
"watch-fitness",
"watch-calculator",
"watch-apple",
"washing-machine",
"warehouse-full",
"wand-sparkles",
"wand-magic",
"wand",
"walker",
"waffle",
"w",
"vr-cardboard",
"volume-xmark",
"volume-slash",
"volume-off",
"volume-low",
"volume-high",
"volleyball",
"volcano",
"virus-slash",
"virus-covid-slash",
"virus-covid",
"violin",
"vihara",
"video-plus",
"video-arrow-up-right",
"video-arrow-down-left",
"vial-virus",
"vial-circle-check",
"vial",
"vest-patches",
"vest",
"venus-mars",
"venus-double",
"venus",
"vent-damper",
"vector-square",
"vector-polygon",
"vector-circle",
"vault",
"van-shuttle",
"value-absolute",
"vacuum-robot",
"vacuum",
"v",
"utility-pole-double",
"utility-pole",
"utensils-slash",
"utensils",
"users-viewfinder",
"users-slash",
"users-rectangle",
"users-rays",
"users-line",
"users-gear",
"users-between-lines",
"user-xmark",
"user-vneck-hair-long",
"user-vneck-hair",
"user-vneck",
"user-visor",
"user-unlock",
"user-tie-hair-long",
"user-tie-hair",
"user-tag",
"user-slash",
"user-shield",
"user-shakespeare",
"user-robot-xmarks",
"user-robot",
"user-police-tie",
"user-police",
"user-pilot-tie",
"user-pilot",
"user-pen",
"user-nurse-hair-long",
"user-nurse-hair",
"user-ninja",
"user-music",
"user-magnifying-glass",
"user-lock",
"user-large-slash",
"user-large",
"user-injured",
"user-helmet-safety",
"user-headset",
"user-hair-mullet",
"user-hair-long",
"user-hair-buns",
"user-hair",
"user-group-crown",
"user-group",
"user-graduate",
"user-gear",
"user-doctor-message",
"user-doctor-hair-long",
"user-doctor-hair",
"user-doctor",
"user-crown",
"user-cowboy",
"user-clock",
"user-chef",
"user-check",
"user-astronaut",
"user-alien",
"usb-drive",
"up-to-line",
"up-to-dotted-line",
"up-right-from-square",
"up-right-and-down-left-from-center",
"up-right",
"up-long",
"up-left",
"up-from-line",
"up-from-dotted-line",
"up-from-bracket",
"up-down-left-right",
"up-down",
"up",
"unlock-keyhole",
"universal-access",
"union",
"uniform-martial-arts",
"unicorn",
"underline",
"umbrella-simple",
"ufo-beam",
"u",
"tv-retro",
"tv-music",
"tv",
"turtle",
"turntable",
"turn-up",
"turn-down-right",
"turn-down-left",
"turn-down",
"turkish-lira-sign",
"turkey",
"tugrik-sign",
"tty-answer",
"trumpet",
"truck-tow",
"truck-ramp-couch",
"truck-ramp-box",
"truck-ramp",
"truck-plow",
"truck-plane",
"truck-pickup",
"truck-moving",
"truck-medical",
"truck-flatbed",
"truck-field-un",
"truck-field",
"truck-droplet",
"truck-container-empty",
"truck-container",
"truck-clock",
"truck-bolt",
"truck-arrow-right",
"trowel-bricks",
"trowel",
"trophy-star",
"trillium",
"tricycle-adult",
"tricycle",
"triangle-person-digging",
"triangle-instrument",
"triangle-exclamation",
"trees",
"tree-palm",
"tree-large",
"tree-decorated",
"tree-deciduous",
"tree-city",
"tree-christmas",
"treasure-chest",
"trash-xmark",
"trash-undo",
"trash-slash",
"trash-plus",
"trash-list",
"trash-clock",
"trash-check",
"trash-can-xmark",
"trash-can-undo",
"trash-can-slash",
"trash-can-plus",
"trash-can-list",
"trash-can-clock",
"trash-can-check",
"trash-can-arrow-up",
"trash-can",
"trash-arrow-up",
"transporter-7",
"transporter-6",
"transporter-5",
"transporter-4",
"transporter-2",
"transporter-1",
"transgender",
"transformer-bolt",
"train-tunnel",
"train-tram",
"train-track",
"train-subway-tunnel",
"train-subway",
"trailer",
"traffic-light-stop",
"traffic-light-slow",
"traffic-light-go",
"traffic-light",
"trademark",
"tractor",
"tower-observation",
"tower-control",
"tower-cell",
"tower-broadcast",
"tornado",
"torii-gate",
"toothbrush",
"tooth",
"toolbox",
"tombstone-blank",
"tombstone",
"tomato",
"toilets-portable",
"toilet-portable",
"toilet-paper-xmark",
"toilet-paper-under-slash",
"toilet-paper-under",
"toilet-paper-slash",
"toilet-paper-check",
"toilet-paper-blank-under",
"toilet-paper-blank",
"toilet-paper",
"toilet",
"toggle-off",
"toggle-large-on",
"toggle-large-off",
"tire-rugged",
"tire-pressure-warning",
"tire-flat",
"tire",
"timer",
"timeline-arrow",
"timeline",
"tilde",
"tickets-airline",
"ticket-simple",
"ticket-airline",
"tick",
"theta",
"thermometer",
"text-width",
"text-slash",
"text-height",
"terminal",
"tents",
"tent-arrows-down",
"tent-arrow-turn-left",
"tent-arrow-left-right",
"tent-arrow-down-to-line",
"tent",
"tennis-ball",
"tenge-sign",
"temperature-three-quarters",
"temperature-sun",
"temperature-snow",
"temperature-quarter",
"temperature-low",
"temperature-list",
"temperature-high",
"temperature-half",
"temperature-full",
"temperature-empty",
"temperature-arrow-up",
"temperature-arrow-down",
"telescope",
"teeth-open",
"teeth",
"teddy-bear",
"taxi-bus",
"taxi",
"tarp-droplet",
"tarp",
"tape",
"tank-water",
"tamale",
"tally-4",
"tally-3",
"tally-2",
"tally-1",
"taco",
"tachograph-digital",
"tablet-screen-button",
"tablet-screen",
"tablet-button",
"table-tree",
"table-tennis-paddle-ball",
"table-rows",
"table-pivot",
"table-picnic",
"table-list",
"table-layout",
"table-columns",
"table-cells-large",
"table-cells",
"t",
"syringe",
"synagogue",
"symbols",
"swords-laser",
"swords",
"sword-laser-alt",
"sword-laser",
"sword",
"swatchbook",
"sushi-roll",
"sushi",
"superscript",
"sunset",
"sunrise",
"sunglasses",
"sun-plant-wilt",
"sun-haze",
"sun-dust",
"sun-cloud",
"sun-bright",
"suitcase-rolling",
"suitcase-medical",
"suitcase",
"stroopwafel",
"stretcher",
"store-slash",
"store-lock",
"stopwatch-20",
"stopwatch",
"stomach",
"sterling-sign",
"steering-wheel",
"steak",
"starship",
"starfighter-twin-ion-engine-advanced",
"starfighter-twin-ion-engine",
"starfighter",
"star-shooting",
"star-sharp-half-stroke",
"star-sharp-half",
"star-sharp",
"star-of-david",
"star-half-stroke",
"star-half",
"star-exclamation",
"star-christmas",
"star-and-crescent",
"stapler",
"standard-definition",
"stairs",
"staff-snake",
"squirrel",
"squid",
"square-z",
"square-y",
"square-x",
"square-w",
"square-virus",
"square-v",
"square-user",
"square-up-right",
"square-up-left",
"square-up",
"square-u",
"square-this-way-up",
"square-terminal",
"square-t",
"square-star",
"square-small",
"square-sliders-vertical",
"square-sliders",
"square-share-nodes",
"square-s",
"square-rss",
"square-root-variable",
"square-root",
"square-ring",
"square-right",
"square-r",
"square-quote",
"square-quarters",
"square-q",
"square-poll-vertical",
"square-poll-horizontal",
"square-plus",
"square-phone-hangup",
"square-phone-flip",
"square-phone",
"square-person-confined",
"square-pen",
"square-parking-slash",
"square-parking",
"square-p",
"square-o",
"square-nfi",
"square-n",
"square-m",
"square-list",
"square-left",
"square-l",
"square-kanban",
"square-k",
"square-j",
"square-i",
"square-h",
"square-g",
"square-full",
"square-fragile",
"square-f",
"square-exclamation",
"square-envelope",
"square-ellipsis-vertical",
"square-ellipsis",
"square-e",
"square-down-right",
"square-down-left",
"square-down",
"square-dollar",
"square-divide",
"square-dashed-circle-plus",
"square-dashed",
"square-d",
"square-code",
"square-chevron-up",
"square-chevron-right",
"square-chevron-left",
"square-chevron-down",
"square-caret-up",
"square-caret-right",
"square-caret-left",
"square-caret-down",
"square-c",
"square-bolt",
"square-b",
"square-arrow-up-right",
"square-arrow-up-left",
"square-arrow-up",
"square-arrow-right",
"square-arrow-left",
"square-arrow-down-right",
"square-arrow-down-left",
"square-arrow-down",
"square-ampersand",
"square-a-lock",
"square-a",
"square-9",
"square-8",
"square-7",
"square-6",
"square-5",
"square-4",
"square-3",
"square-2",
"square-1",
"square-0",
"sprinkler-ceiling",
"sprinkler",
"spray-can-sparkles",
"spray-can",
"sportsball",
"spoon",
"splotch",
"split",
"spider-web",
"spider-black-widow",
"spider",
"spell-check",
"sparkle",
"spaghetti-monster-flying",
"spade",
"space-station-moon-construction",
"space-station-moon",
"spa",
"sort-up",
"sort-down",
"soft-serve",
"socks",
"snowplow",
"snowman-head",
"snowman",
"snowflakes",
"snowflake-droplets",
"snow-blowing",
"snooze",
"snake",
"smoking",
"smoke",
"smog",
"slot-machine",
"sliders-up",
"sliders-simple",
"slider",
"sleigh",
"slash-forward",
"slash-back",
"skull-crossbones",
"skull-cow",
"skull",
"ski-boot-ski",
"ski-boot",
"skeleton-ribs",
"skeleton",
"siren-on",
"siren",
"sink",
"sim-cards",
"sim-card",
"signs-post",
"signature-slash",
"signature-lock",
"signal-weak",
"signal-strong",
"signal-stream-slash",
"signal-stream",
"signal-good",
"signal-fair",
"signal-bars-weak",
"signal-bars-slash",
"signal-bars-good",
"signal-bars-fair",
"sign-hanging",
"sigma",
"sidebar-flip",
"sidebar",
"sickle",
"shuttlecock",
"shuttle-space",
"shutters",
"shuffle",
"shrimp",
"shredder",
"shower-down",
"shovel-snow",
"shovel",
"shop-slash",
"shop-lock",
"shoe-prints",
"shish-kebab",
"shirt-tank-top",
"shirt-running",
"shirt-long-sleeve",
"shield-xmark",
"shield-virus",
"shield-slash",
"shield-quartered",
"shield-plus",
"shield-minus",
"shield-keyhole",
"shield-heart",
"shield-exclamation",
"shield-dog",
"shield-cross",
"shield-cat",
"shelves-empty",
"shelves",
"shekel-sign",
"sheet-plastic",
"sheep",
"share-nodes",
"share-all",
"shapes",
"sensor-triangle-exclamation",
"sensor-on",
"sensor-fire",
"sensor-cloud",
"sensor",
"send-backward",
"send-back",
"semicolon",
"section",
"seat-airline",
"seal-question",
"seal-exclamation",
"seal",
"sd-cards",
"sd-card",
"scythe",
"scrubber",
"scroll-torah",
"scroll-old",
"scribble",
"screwdriver-wrench",
"screwdriver",
"screencast",
"screen-users",
"scissors",
"school-lock",
"school-flag",
"school-circle-xmark",
"school-circle-exclamation",
"school-circle-check",
"scarf",
"scarecrow",
"scanner-keyboard",
"scanner-image",
"scanner-gun",
"scalpel-line-dashed",
"scalpel",
"scale-unbalanced-flip",
"scale-unbalanced",
"scale-balanced",
"saxophone-fire",
"saxophone",
"sausage",
"satellite-dish",
"satellite",
"sandwich",
"salt-shaker",
"salad",
"sailboat",
"sack-xmark",
"sack-dollar",
"sack",
"s",
"rv",
"rupiah-sign",
"rupee-sign",
"ruler-triangle",
"ruler-horizontal",
"ruler-combined",
"rugby-ball",
"rug",
"ruble-sign",
"rss",
"router",
"route-interstate",
"route-highway",
"rotate-left",
"rotate-exclamation",
"rotate",
"roller-coaster",
"road-spikes",
"road-lock",
"road-circle-xmark",
"road-circle-exclamation",
"road-circle-check",
"road-bridge",
"road-barrier",
"rings-wedding",
"ring-diamond",
"right-to-line",
"right-long-to-line",
"right-long",
"right-left",
"right-from-line",
"right-from-bracket",
"right",
"ribbon",
"rhombus",
"retweet",
"restroom-simple",
"republican",
"reply-clock",
"reply-all",
"repeat-1",
"refrigerator",
"reel",
"rectangles-mixed",
"rectangle-xmark",
"rectangle-vertical-history",
"rectangle-vertical",
"rectangle-terminal",
"rectangle-pro",
"rectangle-list",
"rectangle-history-circle-user",
"rectangle-history-circle-plus",
"rectangle-history",
"rectangle-code",
"rectangle-barcode",
"rectangle-ad",
"rectangle",
"ranking-star",
"ramp-loading",
"ram",
"rainbow",
"radio-tuner",
"radiation",
"racquet",
"rabbit-running",
"rabbit",
"r",
"quotes",
"qrcode",
"q",
"puzzle-piece-simple",
"puzzle",
"pumpkin",
"pump-soap",
"pump-medical",
"pump",
"print-slash",
"print-magnifying-glass",
"pretzel",
"presentation-screen",
"prescription-bottle-pill",
"prescription-bottle-medical",
"prescription-bottle",
"prescription",
"potato",
"pot-food",
"popsicle",
"popcorn",
"poop",
"pool-8-ball",
"pompebled",
"poll-people",
"podium-star",
"podium",
"podcast",
"plus-minus",
"plus-large",
"plug-circle-xmark",
"plug-circle-plus",
"plug-circle-minus",
"plug-circle-exclamation",
"plug-circle-check",
"plug-circle-bolt",
"plate-wheat",
"plate-utensils",
"plant-wilt",
"plane-up-slash",
"plane-up",
"plane-tail",
"plane-slash",
"plane-prop",
"plane-lock",
"plane-engines",
"plane-circle-xmark",
"plane-circle-exclamation",
"plane-circle-check",
"plane-arrival",
"pizza-slice",
"pizza",
"pipe-valve",
"pipe-smoking",
"pipe-section",
"pipe-collar",
"pipe-circle-check",
"pipe",
"pineapple",
"pinball",
"pinata",
"pills",
"piggy-bank",
"pig",
"pie",
"pickleball",
"pickaxe",
"piano-keyboard",
"piano",
"pi",
"photo-film-music",
"photo-film",
"phone-xmark",
"phone-slash",
"phone-rotary",
"phone-plus",
"phone-missed",
"phone-intercom",
"phone-hangup",
"phone-flip",
"phone-arrow-up-right",
"phone-arrow-right",
"phone-arrow-down-left",
"peso-sign",
"peseta-sign",
"person-walking-with-cane",
"person-walking-luggage",
"person-walking-dashed-line-arrow-right",
"person-walking-arrow-right",
"person-walking-arrow-loop-left",
"person-walking",
"person-to-portal",
"person-to-door",
"person-through-window",
"person-swimming",
"person-snowmobiling",
"person-snowboarding",
"person-sledding",
"person-skiing-nordic",
"person-skiing",
"person-ski-lift",
"person-ski-jumping",
"person-skating",
"person-simple",
"person-shelter",
"person-seat-reclined",
"person-seat",
"person-running",
"person-rifle",
"person-rays",
"person-pregnant",
"person-praying",
"person-pinball",
"person-military-to-person",
"person-military-rifle",
"person-military-pointing",
"person-hiking",
"person-harassing",
"person-half-dress",
"person-from-portal",
"person-falling-burst",
"person-falling",
"person-drowning",
"person-dress-simple",
"person-dress-burst",
"person-dots-from-line",
"person-dolly",
"person-digging",
"person-circle-xmark",
"person-circle-question",
"person-circle-plus",
"person-circle-minus",
"person-circle-exclamation",
"person-circle-check",
"person-chalkboard",
"person-carry-box",
"person-cane",
"person-burst",
"person-breastfeeding",
"person-booth",
"person-biking-mountain",
"person-biking",
"person-arrow-up-from-line",
"person-arrow-down-to-line",
"period",
"pepper-hot",
"pepper",
"people-roof",
"people-robbery",
"people-pulling",
"people-pants-simple",
"people-pants",
"people-line",
"people-group",
"people-dress-simple",
"people-dress",
"people-carry-box",
"people",
"pencil-slash",
"pencil-mechanical",
"pen-swirl",
"pen-slash",
"pen-ruler",
"pen-paintbrush",
"pen-nib-slash",
"pen-line",
"pen-field",
"pen-fancy-slash",
"pen-clip-slash",
"pen-clip",
"pen-circle",
"pegasus",
"pedestal",
"pear",
"peapod",
"peanuts",
"peanut",
"peach",
"peace",
"paw-simple",
"paw-claws",
"paw",
"paste",
"party-horn",
"party-bell",
"paragraph-left",
"paragraph",
"parachute-box",
"paperclip-vertical",
"paper-plane-top",
"panorama",
"panel-fire",
"panel-ews",
"pancakes",
"pan-frying",
"pan-food",
"pallet-boxes",
"pallet-box",
"pallet",
"paintbrush-pencil",
"paintbrush-fine",
"paintbrush",
"pager",
"page-caret-up",
"page-caret-down",
"page",
"p",
"overline",
"oven",
"outlet",
"outdent",
"ornament",
"option",
"onion",
"omega",
"om",
"olive-branch",
"olive",
"oil-well",
"oil-temperature",
"oil-can-drip",
"oil-can",
"octagon-xmark",
"octagon-plus",
"octagon-minus",
"octagon-exclamation",
"octagon-divide",
"octagon-check",
"octagon",
"objects-column",
"objects-align-top",
"objects-align-right",
"objects-align-left",
"objects-align-center-vertical",
"objects-align-center-horizontal",
"objects-align-bottom",
"object-union",
"object-ungroup",
"object-subtract",
"object-intersect",
"object-group",
"object-exclude",
"o",
"notes",
"notebook",
"note-sticky",
"note-medical",
"note",
"notdef",
"nose",
"nfc-trash",
"nfc-symbol",
"nfc-slash",
"nfc-signal",
"nfc-pen",
"nfc-magnifying-glass",
"nfc-lock",
"nfc",
"neuter",
"nesting-dolls",
"narwhal",
"naira-sign",
"n",
"mustache",
"music-slash",
"music-note-slash",
"music-note",
"mushroom",
"mug-tea-saucer",
"mug-marshmallows",
"mug",
"mp3-player",
"mouse-field",
"mountains",
"mountain-city",
"mountain",
"mound",
"mosquito-net",
"mosquito",
"mosque",
"mortar-pestle",
"moon-over-sun",
"moon-cloud",
"monument",
"monkey",
"monitor-waveform",
"money-simple-from-bracket",
"money-from-bracket",
"money-check-pen",
"money-check-dollar-pen",
"money-check-dollar",
"money-check",
"money-bills-simple",
"money-bills",
"money-bill-wheat",
"money-bill-wave",
"money-bill-trend-up",
"money-bill-transfer",
"money-bill-simple-wave",
"money-bill-simple",
"money-bill-1-wave",
"money-bill-1",
"mobile-signal-out",
"mobile-signal",
"mobile-screen-button",
"mobile-screen",
"mobile-retro",
"mobile-notch",
"mobile-button",
"mitten",
"mistletoe",
"minimize",
"mill-sign",
"microscope",
"microphone-stand",
"microphone-lines-slash",
"microphone-lines",
"microchip-ai",
"meter-fire",
"meter-droplet",
"meter-bolt",
"meter",
"meteor",
"messages-question",
"messages-dollar",
"messages",
"message-xmark",
"message-text",
"message-sms",
"message-smile",
"message-slash",
"message-quote",
"message-question",
"message-plus",
"message-pen",
"message-music",
"message-minus",
"message-middle-top",
"message-middle",
"message-medical",
"message-lines",
"message-image",
"message-heart",
"message-exclamation",
"message-dots",
"message-dollar",
"message-code",
"message-check",
"message-captions",
"message-bot",
"message-arrow-up-right",
"message-arrow-up",
"message-arrow-down",
"message",
"merge",
"mercury",
"menorah",
"memory",
"memo-pad",
"memo-circle-info",
"memo-circle-check",
"memo",
"melon-slice",
"melon",
"maximize",
"mattress-pillow",
"masks-theater",
"mask-ventilator",
"mask-snorkel",
"mask-face",
"martini-glass-empty",
"martini-glass-citrus",
"martini-glass",
"mars-stroke-up",
"mars-stroke-right",
"mars-stroke",
"mars-double",
"mars-and-venus-burst",
"mars-and-venus",
"mars",
"map-pin",
"map-location-dot",
"map-location",
"map",
"manhole",
"mango",
"mandolin",
"manat-sign",
"mailbox-flag-up",
"magnifying-glass-plus",
"magnifying-glass-minus",
"magnifying-glass-location",
"magnifying-glass-dollar",
"magnifying-glass-chart",
"magnifying-glass-arrow-right",
"mace",
"m",
"lungs-virus",
"lungs",
"luchador-mask",
"lollipop",
"locust",
"lock-open",
"lock-keyhole-open",
"lock-keyhole",
"lock-hashtag",
"lock-a",
"location-xmark",
"location-smile",
"location-question",
"location-plus",
"location-pin-slash",
"location-pin-lock",
"location-pen",
"location-minus",
"location-exclamation",
"location-dot-slash",
"location-crosshairs-slash",
"location-crosshairs",
"location-check",
"location-arrow",
"lobster",
"loader",
"litecoin-sign",
"list-ul",
"list-tree",
"list-timeline",
"list-radio",
"list-ol",
"list-music",
"list-dropdown",
"list-check",
"lira-sign",
"lips",
"link-slash",
"link-simple-slash",
"link-simple",
"link-horizontal-slash",
"link-horizontal",
"lines-leaning",
"line-columns",
"lightbulb-on",
"lightbulb-exclamation-on",
"lightbulb-exclamation",
"lightbulb-dollar",
"lightbulb-cfl-on",
"lightbulb-cfl",
"light-switch-on",
"light-switch-off",
"light-emergency-on",
"light-emergency",
"light-ceiling",
"life-ring",
"less-than-equal",
"less-than",
"left-to-line",
"left-right",
"left-long-to-line",
"left-long",
"left-from-line",
"left",
"leafy-green",
"leaf-oak",
"leaf-maple",
"leaf-heart",
"layer-plus",
"layer-minus",
"lasso-sparkles",
"lasso",
"lari-sign",
"laptop-slash",
"laptop-mobile",
"laptop-medical",
"laptop-file",
"laptop-code",
"laptop-arrow-down",
"landmark-flag",
"landmark-dome",
"land-mine-on",
"lamp-street",
"lamp-floor",
"lamp-desk",
"lambda",
"lacrosse-stick-ball",
"lacrosse-stick",
"l",
"knife",
"kiwi-fruit",
"kite",
"kitchen-set",
"kit-medical",
"kip-sign",
"khanda",
"keynote",
"keyboard-left",
"keyboard-down",
"keyboard-brightness-low",
"keyboard-brightness",
"key-skeleton-left-right",
"key-skeleton",
"kerning",
"kazoo",
"kaaba",
"k",
"jug-detergent",
"jug",
"jet-fighter-up",
"jet-fighter",
"jedi",
"jar-wheat",
"jar",
"jack-o-lantern",
"j",
"italic",
"island-tropical",
"intersection",
"interrobang",
"integral",
"input-text",
"input-pipe",
"input-numeric",
"inhaler",
"infinity",
"industry-windows",
"indian-rupee-sign",
"indent",
"inboxes",
"inbox-out",
"inbox-in",
"inbox-full",
"images-user",
"image-user",
"image-slash",
"image-portrait",
"image-polaroid-user",
"image-polaroid",
"image-landscape",
"igloo",
"id-card-clip",
"id-card",
"id-badge",
"icicles",
"ice-skate",
"ice-cream",
"i",
"hyphen",
"hurricane",
"hundred-points",
"hryvnia-sign",
"house-window",
"house-water",
"house-user",
"house-turret",
"house-tsunami",
"house-tree",
"house-signal",
"house-person-return",
"house-person-leave",
"house-medical-flag",
"house-medical-circle-xmark",
"house-medical-circle-exclamation",
"house-medical-circle-check",
"house-medical",
"house-lock",
"house-laptop",
"house-heart",
"house-flood-water-circle-arrow-right",
"house-flood-water",
"house-flag",
"house-fire",
"house-day",
"house-crack",
"house-circle-xmark",
"house-circle-exclamation",
"house-circle-check",
"house-chimney-window",
"house-chimney-user",
"house-chimney-medical",
"house-chimney-heart",
"house-chimney-crack",
"house-chimney-blank",
"house-chimney",
"house-building",
"house-blank",
"hourglass-half",
"hourglass-end",
"hourglass-clock",
"hourglass",
"hotdog",
"hot-tub-person",
"hospitals",
"hospital-user",
"hose-reel",
"hose",
"horse-saddle",
"horse-head",
"horse",
"horizontal-rule",
"hood-cloak",
"honey-pot",
"holly-berry",
"hockey-sticks",
"hockey-stick-puck",
"hockey-puck",
"hockey-mask",
"hill-rockslide",
"hill-avalanche",
"highlighter-line",
"highlighter",
"high-definition",
"hexagon-xmark",
"hexagon-vertical-nft-slanted",
"hexagon-vertical-nft",
"hexagon-plus",
"hexagon-minus",
"hexagon-image",
"hexagon-exclamation",
"hexagon-divide",
"hexagon-check",
"hexagon",
"helmet-un",
"helmet-safety",
"helmet-battle",
"helicopter-symbol",
"helicopter",
"heat",
"heart-pulse",
"heart-half-stroke",
"heart-half",
"heart-crack",
"heart-circle-xmark",
"heart-circle-plus",
"heart-circle-minus",
"heart-circle-exclamation",
"heart-circle-check",
"heart-circle-bolt",
"headphones-simple",
"head-side-virus",
"head-side-medical",
"head-side-mask",
"head-side-heart",
"head-side-headphones",
"head-side-goggles",
"head-side-cough-slash",
"head-side-cough",
"head-side-brain",
"hat-wizard",
"hat-witch",
"hat-winter",
"hat-santa",
"hat-cowboy-side",
"hat-cowboy",
"hat-chef",
"hashtag-lock",
"hard-drive",
"hanukiah",
"handshake-simple-slash",
"handshake-simple",
"handshake-angle",
"hands-praying",
"hands-holding-heart",
"hands-holding-dollar",
"hands-holding-diamond",
"hands-holding-circle",
"hands-holding-child",
"hands-holding",
"hands-clapping",
"hands-bubbles",
"hands-bound",
"hands-asl-interpreting",
"handcuffs",
"hand-wave",
"hand-spock",
"hand-sparkles",
"hand-scissors",
"hand-pointer",
"hand-point-right",
"hand-point-ribbon",
"hand-point-left",
"hand-point-down",
"hand-peace",
"hand-middle-finger",
"hand-love",
"hand-lizard",
"hand-horns",
"hand-holding-skull",
"hand-holding-seedling",
"hand-holding-medical",
"hand-holding-magic",
"hand-holding-hand",
"hand-holding-droplet",
"hand-holding-dollar",
"hand-holding-box",
"hand-holding",
"hand-heart",
"hand-fist",
"hand-fingers-crossed",
"hand-dots",
"hand-back-point-up",
"hand-back-point-right",
"hand-back-point-ribbon",
"hand-back-point-left",
"hand-back-point-down",
"hand-back-fist",
"hamsa",
"hammer-war",
"hammer-crash",
"h6",
"h5",
"h4",
"h3",
"h2",
"h1",
"h",
"gun-squirt",
"gun-slash",
"gun",
"guitars",
"guitar-electric",
"guitar",
"guarani-sign",
"group-arrows-rotate",
"grip-vertical",
"grip-lines-vertical",
"grip-lines",
"grip-dots-vertical",
"grip-dots",
"grip",
"grill-fire",
"grill",
"grid-horizontal",
"grid-dividers",
"grid-5",
"grid-4",
"grid-2-plus",
"grid",
"greater-than-equal",
"greater-than",
"grate-droplet",
"grate",
"grapes",
"gramophone",
"graduation-cap",
"golf-flag-hole",
"golf-club",
"golf-ball-tee",
"goal-net",
"globe-snow",
"glasses-round",
"glass-water-droplet",
"glass-water",
"glass-half",
"glass-empty",
"glass-citrus",
"gingerbread-man",
"gifts",
"gift-card",
"gif",
"genderless",
"gem",
"gavel",
"gauge-simple-min",
"gauge-simple-max",
"gauge-simple-low",
"gauge-simple-high",
"gauge-simple",
"gauge-min",
"gauge-max",
"gauge-low",
"gauge-high",
"gauge-circle-plus",
"gauge-circle-minus",
"gauge-circle-bolt",
"gas-pump-slash",
"gas-pump",
"garlic",
"garage-open",
"garage-car",
"garage",
"gamepad-modern",
"game-console-handheld-crank",
"game-board-simple",
"game-board",
"gallery-thumbnails",
"galaxy",
"g",
"futbol",
"function",
"frog",
"french-fries",
"franc-sign",
"frame",
"forward-step",
"forward-fast",
"forklift",
"fork-knife",
"fork",
"football-helmet",
"football",
"font-awesome",
"fondue-pot",
"folders",
"folder-xmark",
"folder-user",
"folder-tree",
"folder-plus",
"folder-music",
"folder-minus",
"folder-medical",
"folder-magnifying-glass",
"folder-image",
"folder-heart",
"folder-grid",
"folder-gear",
"folder-closed",
"folder-bookmark",
"folder-arrow-up",
"folder-arrow-down",
"flying-disc",
"flux-capacitor",
"flute",
"flower-daffodil",
"florin-sign",
"floppy-disks",
"floppy-disk-pen",
"floppy-disk-circle-xmark",
"floppy-disk-circle-arrow-right",
"floppy-disk",
"flatbread-stuffed",
"flatbread",
"flask-vial",
"flask-round-potion",
"flask-round-poison",
"flashlight",
"flag-usa",
"flag-swallowtail",
"flag-pennant",
"fishing-rod",
"fish-fins",
"fish-cooked",
"fish-bones",
"fireplace",
"fire-smoke",
"fire-hydrant",
"fire-flame-simple",
"fire-flame-curved",
"fire-flame",
"fire-extinguisher",
"fire-burner",
"filters",
"filter-slash",
"filter-list",
"filter-circle-xmark",
"filter-circle-dollar",
"films",
"film-slash",
"film-simple",
"film-canister",
"fill-drip",
"fill",
"files-medical",
"file-zipper",
"file-xmark",
"file-waveform",
"file-video",
"file-user",
"file-slash",
"file-signature",
"file-shield",
"file-prescription",
"file-powerpoint",
"file-plus-minus",
"file-plus",
"file-pen",
"file-music",
"file-minus",
"file-medical",
"file-magnifying-glass",
"file-lock",
"file-lines",
"file-invoice-dollar",
"file-image",
"file-heart",
"file-dashed-line",
"file-csv",
"file-code",
"file-circle-xmark",
"file-circle-question",
"file-circle-plus",
"file-circle-minus",
"file-circle-info",
"file-circle-exclamation",
"file-circle-check",
"file-chart-pie",
"file-chart-column",
"file-certificate",
"file-binary",
"file-audio",
"file-arrow-up",
"file-arrow-down",
"field-hockey-stick-ball",
"ferry",
"ferris-wheel",
"fence",
"feather-pointed",
"fax",
"faucet-drip",
"faucet",
"farm",
"fan-table",
"fan",
"family-pants",
"family-dress",
"family",
"falafel",
"face-zipper",
"face-zany",
"face-worried",
"face-woozy",
"face-weary",
"face-viewfinder",
"face-unamused",
"face-tongue-sweat",
"face-tongue-money",
"face-tissue",
"face-tired",
"face-thinking",
"face-thermometer",
"face-swear",
"face-surprise",
"face-sunglasses",
"face-spiral-eyes",
"face-smirking",
"face-smiling-hands",
"face-smile-wink",
"face-smile-upside-down",
"face-smile-tongue",
"face-smile-tear",
"face-smile-relaxed",
"face-smile-plus",
"face-smile-horns",
"face-smile-hearts",
"face-smile-halo",
"face-smile-beam",
"face-sleepy",
"face-sleeping",
"face-shush",
"face-scream",
"face-saluting",
"face-sad-tear",
"face-sad-sweat",
"face-sad-cry",
"face-rolling-eyes",
"face-relieved",
"face-raised-eyebrow",
"face-pouting",
"face-pleading",
"face-persevering",
"face-pensive",
"face-party",
"face-nose-steam",
"face-nauseated",
"face-monocle",
"face-melting",
"face-meh-blank",
"face-meh",
"face-mask",
"face-lying",
"face-laugh-wink",
"face-laugh-squint",
"face-laugh-beam",
"face-laugh",
"face-kiss-wink-heart",
"face-kiss-closed-eyes",
"face-kiss-beam",
"face-kiss",
"face-icicles",
"face-hushed",
"face-holding-back-tears",
"face-head-bandage",
"face-hand-yawn",
"face-hand-peeking",
"face-hand-over-mouth",
"face-grin-wink",
"face-grin-wide",
"face-grin-tongue-wink",
"face-grin-tongue-squint",
"face-grin-tongue",
"face-grin-tears",
"face-grin-stars",
"face-grin-squint-tears",
"face-grin-squint",
"face-grin-hearts",
"face-grin-beam-sweat",
"face-grin-beam",
"face-grin",
"face-grimace",
"face-glasses",
"face-frown-slight",
"face-frown-open",
"face-frown",
"face-flushed",
"face-fearful",
"face-eyes-xmarks",
"face-expressionless",
"face-explode",
"face-exhaling",
"face-drooling",
"face-downcast-sweat",
"face-dotted",
"face-dizzy",
"face-disguise",
"face-disappointed",
"face-diagonal-mouth",
"face-cowboy-hat",
"face-confused",
"face-confounded",
"face-clouds",
"face-beam-hand-over-mouth",
"face-astonished",
"face-anxious-sweat",
"face-anguished",
"face-angry-horns",
"face-angry",
"f",
"eyes",
"eye-low-vision",
"eye-evil",
"eye-dropper-half",
"eye-dropper-full",
"eye-dropper",
"explosion",
"expand-wide",
"euro-sign",
"ethernet",
"escalator",
"equals",
"envelopes-bulk",
"envelope-open-text",
"envelope-open-dollar",
"envelope-open",
"envelope-dot",
"envelope-circle-check",
"engine",
"ellipsis-vertical",
"ellipsis-stroke-vertical",
"ellipsis-stroke",
"ellipsis",
"elevator",
"elephant",
"eject",
"eggplant",
"egg-fried",
"egg",
"eclipse",
"earth-oceania",
"earth-europe",
"earth-asia",
"earth-africa",
"ear-muffs",
"ear-listen",
"ear-deaf",
"ear",
"e",
"dungeon",
"dumpster",
"duck",
"dryer-heat",
"dryer",
"drumstick-bite",
"drumstick",
"drum-steelpan",
"drum",
"droplet-slash",
"droplet-percent",
"droplet-degree",
"drone-front",
"drone",
"dreidel",
"draw-polygon",
"draw-circle",
"down-to-line",
"down-to-dotted-line",
"down-to-bracket",
"down-right",
"down-long",
"down-left-and-up-right-to-center",
"down-left",
"down-from-line",
"down-from-dotted-line",
"down",
"dove",
"door-closed",
"donut",
"dong-sign",
"dolly-empty",
"dolly",
"dog-leashed",
"dog",
"do-not-enter",
"dna",
"divide",
"ditto",
"distribute-spacing-vertical",
"distribute-spacing-horizontal",
"display-slash",
"display-medical",
"display-code",
"display-arrow-down",
"display",
"disease",
"disc-drive",
"diploma",
"dice-two",
"dice-three",
"dice-six",
"dice-one",
"dice-four",
"dice-five",
"dice-d8",
"dice-d6",
"dice-d4",
"dice-d20",
"dice-d12",
"dice-d10",
"diamond-turn-right",
"diamond-half-stroke",
"diamond-half",
"diamond-exclamation",
"dial-off",
"dial-min",
"dial-med-low",
"dial-med",
"dial-max",
"dial-low",
"dial-high",
"dial",
"diagram-successor",
"diagram-subtask",
"diagram-sankey",
"diagram-project",
"diagram-previous",
"diagram-predecessor",
"diagram-next",
"diagram-nested",
"diagram-lean-canvas",
"diagram-cells",
"dharmachakra",
"desktop-arrow-down",
"democrat",
"delete-right",
"delete-left",
"deer-rudolph",
"deer",
"dash",
"dagger",
"d",
"custard",
"curling-stone",
"cupcake",
"cup-togo",
"cup-straw-swoosh",
"cup-straw",
"cucumber",
"cubes-stacked",
"cubes",
"crystal-ball",
"cruzeiro-sign",
"crutches",
"crutch",
"crow",
"crosshairs-simple",
"crop-simple",
"crop",
"croissant",
"cricket-bat-ball",
"credit-card-front",
"credit-card-blank",
"crate-empty",
"crate-apple",
"cowbell-circle-plus",
"cowbell",
"cow",
"couch",
"corner",
"corn",
"cookie",
"conveyor-belt-empty",
"conveyor-belt-boxes",
"conveyor-belt",
"computer-mouse",
"computer",
"compress-wide",
"compress",
"compass-slash",
"compass-drafting",
"compact-disc",
"comments-question-check",
"comments-question",
"comment-xmark",
"comment-text",
"comment-sms",
"comment-smile",
"comment-slash",
"comment-quote",
"comment-question",
"comment-plus",
"comment-pen",
"comment-music",
"comment-minus",
"comment-middle-top",
"comment-middle",
"comment-medical",
"comment-lines",
"comment-image",
"comment-heart",
"comment-exclamation",
"comment-dots",
"comment-dollar",
"comment-code",
"comment-check",
"comment-captions",
"comment-arrow-up-right",
"comment-arrow-up",
"comment-arrow-down",
"command",
"comma",
"comet",
"columns-3",
"colon-sign",
"colon",
"coin-vertical",
"coin-front",
"coin-blank",
"coffin-cross",
"coffin",
"coffee-pot",
"coffee-beans",
"coffee-bean",
"code-simple",
"code-pull-request-draft",
"code-pull-request-closed",
"code-pull-request",
"code-merge",
"code-fork",
"code-commit",
"coconut",
"club",
"clover",
"clouds-sun",
"clouds-moon",
"clouds",
"cloud-xmark",
"cloud-word",
"cloud-sun-rain",
"cloud-sun",
"cloud-snow",
"cloud-sleet",
"cloud-slash",
"cloud-showers-water",
"cloud-showers-heavy",
"cloud-showers",
"cloud-rainbow",
"cloud-rain",
"cloud-question",
"cloud-plus",
"cloud-music",
"cloud-moon-rain",
"cloud-moon",
"cloud-minus",
"cloud-meatball",
"cloud-hail-mixed",
"cloud-hail",
"cloud-fog",
"cloud-exclamation",
"cloud-drizzle",
"cloud-check",
"cloud-bolt-sun",
"cloud-bolt-moon",
"cloud-bolt",
"cloud-arrow-down",
"clothes-hanger",
"closed-captioning-slash",
"clock-two-thirty",
"clock-two",
"clock-twelve-thirty",
"clock-twelve",
"clock-three-thirty",
"clock-three",
"clock-ten-thirty",
"clock-ten",
"clock-six-thirty",
"clock-six",
"clock-seven-thirty",
"clock-seven",
"clock-rotate-left",
"clock-one-thirty",
"clock-one",
"clock-nine-thirty",
"clock-nine",
"clock-four-thirty",
"clock-five-thirty",
"clock-five",
"clock-eleven-thirty",
"clock-eleven",
"clock-eight-thirty",
"clock-eight",
"clock-desk",
"clipboard-user",
"clipboard-question",
"clipboard-prescription",
"clipboard-medical",
"clipboard-list-check",
"clipboard-list",
"clipboard-check",
"claw-marks",
"clarinet",
"clapperboard-play",
"clapperboard",
"citrus-slice",
"citrus",
"circle-z",
"circle-yen",
"circle-y",
"circle-x",
"circle-waveform-lines",
"circle-w",
"circle-video",
"circle-v",
"circle-up-right",
"circle-up-left",
"circle-u",
"circle-trash",
"circle-three-quarters-stroke",
"circle-three-quarters",
"circle-t",
"circle-stop",
"circle-sterling",
"circle-star",
"circle-sort-up",
"circle-sort-down",
"circle-sort",
"circle-small",
"circle-s",
"circle-right",
"circle-radiation",
"circle-r",
"circle-question",
"circle-quarters",
"circle-quarter-stroke",
"circle-quarter",
"circle-q",
"circle-plus",
"circle-play",
"circle-phone-hangup",
"circle-phone-flip",
"circle-phone",
"circle-pause",
"circle-parking",
"circle-p",
"circle-o",
"circle-nodes",
"circle-n",
"circle-minus",
"circle-microphone-lines",
"circle-microphone",
"circle-m",
"circle-location-arrow",
"circle-left",
"circle-l",
"circle-k",
"circle-j",
"circle-i",
"circle-heart",
"circle-half",
"circle-h",
"circle-g",
"circle-f",
"circle-exclamation-check",
"circle-euro",
"circle-envelope",
"circle-ellipsis-vertical",
"circle-ellipsis",
"circle-e",
"circle-down-right",
"circle-down-left",
"circle-dot",
"circle-dollar-to-slot",
"circle-dollar",
"circle-divide",
"circle-dashed",
"circle-d",
"circle-chevron-up",
"circle-chevron-right",
"circle-chevron-left",
"circle-chevron-down",
"circle-caret-up",
"circle-caret-right",
"circle-caret-left",
"circle-caret-down",
"circle-camera",
"circle-calendar",
"circle-c",
"circle-bookmark",
"circle-book-open",
"circle-bolt",
"circle-b",
"circle-arrow-up-right",
"circle-arrow-up-left",
"circle-arrow-up",
"circle-arrow-right",
"circle-arrow-left",
"circle-arrow-down-right",
"circle-arrow-down-left",
"circle-arrow-down",
"circle-ampersand",
"circle-a",
"circle-9",
"circle-8",
"circle-7",
"circle-6",
"circle-5",
"circle-4",
"circle-3",
"circle-2",
"circle-1",
"circle-0",
"chopsticks",
"chimney",
"children",
"child-reaching",
"child-dress",
"child-combatant",
"chevrons-up",
"chevrons-right",
"chevrons-left",
"chevrons-down",
"chevron-right",
"chevron-left",
"chestnut",
"chess-rook-piece",
"chess-rook",
"chess-queen-piece",
"chess-queen",
"chess-pawn-piece",
"chess-pawn",
"chess-knight-piece",
"chess-knight",
"chess-king-piece",
"chess-king",
"chess-clock-flip",
"chess-clock",
"chess-board",
"chess-bishop-piece",
"chess-bishop",
"chess",
"cherries",
"cheese-swiss",
"cheese",
"check-to-slot",
"check-double",
"chart-waterfall",
"chart-user",
"chart-tree-map",
"chart-simple-horizontal",
"chart-scatter-bubble",
"chart-scatter-3d",
"chart-radar",
"chart-pyramid",
"chart-pie-simple",
"chart-pie",
"chart-network",
"chart-mixed",
"chart-line-up",
"chart-line-down",
"chart-line",
"chart-gantt",
"chart-column",
"chart-candlestick",
"chart-bullet",
"chart-bar",
"chart-area",
"charging-station",
"champagne-glasses",
"champagne-glass",
"chalkboard-user",
"chair-office",
"cent-sign",
"cedi-sign",
"cat-space",
"cat",
"castle",
"cassette-vhs",
"cassette-tape",
"cassette-betamax",
"cart-xmark",
"cart-shopping-fast",
"cart-minus",
"cart-flatbed-suitcase",
"cart-flatbed-empty",
"cart-flatbed-boxes",
"cart-flatbed",
"cart-circle-xmark",
"cart-circle-plus",
"cart-circle-exclamation",
"cart-circle-check",
"cart-circle-arrow-up",
"cart-circle-arrow-down",
"cart-arrow-up",
"cart-arrow-down",
"cars",
"carrot",
"caret-right",
"caret-left",
"cards-blank",
"cards",
"card-spade",
"card-heart",
"card-diamond",
"card-club",
"caravan-simple",
"caravan",
"car-wrench",
"car-wash",
"car-tunnel",
"car-tilt",
"car-side-bolt",
"car-rear",
"car-on",
"car-mirrors",
"car-garage",
"car-circle-bolt",
"car-bus",
"car-burst",
"car-bump",
"car-bolt",
"car-battery",
"capsules",
"cannabis",
"candy-corn",
"candy-cane",
"candy-bar",
"candy",
"candle-holder",
"can-food",
"campground",
"camera-web-slash",
"camera-web",
"camera-viewfinder",
"camera-slash",
"camera-security",
"camera-rotate",
"camera-polaroid",
"camera-cctv",
"camcorder",
"calendars",
"calendar-xmark",
"calendar-star",
"calendar-range",
"calendar-plus",
"calendar-pen",
"calendar-minus",
"calendar-lines-pen",
"calendar-lines",
"calendar-image",
"calendar-heart",
"calendar-exclamation",
"calendar-day",
"calendar-clock",
"calendar-circle-user",
"calendar-circle-plus",
"calendar-circle-minus",
"calendar-circle-exclamation",
"calendar-check",
"calendar-arrow-up",
"calendar-arrow-down",
"calculator-simple",
"cake-slice",
"cake-candles",
"cactus",
"cabinet-filing",
"cabin",
"c",
"butter",
"bus-simple",
"bus-school",
"bus",
"burst",
"burger-lettuce",
"burger-glass",
"burger-fries",
"burger-cheese",
"burger",
"buoy-mooring",
"buoy",
"bullseye-arrow",
"bullhorn",
"buildings",
"building-wheat",
"building-user",
"building-un",
"building-shield",
"building-ngo",
"building-lock",
"building-flag",
"building-columns",
"building-circle-xmark",
"building-circle-exclamation",
"building-circle-check",
"building-circle-arrow-right",
"bugs",
"bug-slash",
"bucket",
"browsers",
"broom-wide",
"broom-ball",
"broom",
"broccoli",
"bring-front",
"bring-forward",
"brightness-low",
"brightness",
"briefcase-medical",
"briefcase-blank",
"briefcase-arrow-right",
"bridge-water",
"bridge-suspension",
"bridge-lock",
"bridge-circle-xmark",
"bridge-circle-exclamation",
"bridge-circle-check",
"bridge",
"bread-slice-butter",
"bread-slice",
"bread-loaf",
"brazilian-real-sign",
"brake-warning",
"brain-circuit",
"brain-arrow-curved-right",
"braille",
"brackets-square",
"brackets-round",
"bracket-square-right",
"bracket-square",
"bracket-round-right",
"bracket-round",
"bracket-curly-right",
"bracket-curly",
"boxing-glove",
"boxes-stacked",
"boxes-packing",
"box-tissue",
"box-taped",
"box-open-full",
"box-open",
"box-heart",
"box-dollar",
"box-circle-check",
"box-check",
"box-ballot",
"box-archive",
"box",
"bowling-pins",
"bowling-ball-pin",
"bowling-ball",
"bowl-spoon",
"bowl-soft-serve",
"bowl-scoops",
"bowl-scoop",
"bowl-rice",
"bowl-hot",
"bowl-food",
"bowl-chopsticks",
"bow-arrow",
"bottle-water",
"bottle-droplet",
"bore-hole",
"border-top-left",
"border-right",
"border-outer",
"border-left",
"border-inner",
"border-center-v",
"border-center-h",
"border-bottom-right",
"border-bottom",
"border-all",
"boot-heeled",
"boot",
"boombox",
"books-medical",
"bookmark-slash",
"book-user",
"book-tanakh",
"book-sparkles",
"book-skull",
"book-section",
"book-quran",
"book-open-reader",
"book-open-cover",
"book-open",
"book-medical",
"book-journal-whills",
"book-heart",
"book-font",
"book-copy",
"book-circle-arrow-up",
"book-circle-arrow-right",
"book-bookmark",
"book-blank",
"book-bible",
"book-atlas",
"book-arrow-up",
"book-arrow-right",
"bong",
"bone-break",
"bone",
"bolt-slash",
"bolt-lightning",
"bolt-auto",
"bold",
"bluetooth",
"blueberries",
"block-quote",
"block-brick-fire",
"block-brick",
"block",
"blinds-open",
"blinds",
"blender-phone",
"blender",
"blanket-fire",
"bitcoin-sign",
"biohazard",
"binary-slash",
"binary-lock",
"binary-circle-check",
"binary",
"billboard",
"bezier-curve",
"bench-tree",
"bells",
"bell-slash",
"bell-school-slash",
"bell-school",
"bell-plus",
"bell-on",
"bell-exclamation",
"bell-concierge",
"beer-mug-empty",
"beer-mug",
"bee",
"bed-pulse",
"bed-empty",
"bed-bunk",
"bed",
"battery-three-quarters",
"battery-slash",
"battery-quarter",
"battery-low",
"battery-half",
"battery-full",
"battery-exclamation",
"battery-empty",
"battery-bolt",
"basketball",
"basket-shopping-simple",
"basket-shopping",
"baseball-bat-ball",
"baseball",
"bars-staggered",
"bars-sort",
"bars-progress",
"bars-filter",
"barcode-scan",
"barcode-read",
"banjo",
"bangladeshi-taka-sign",
"bandage",
"banana",
"ban-smoking",
"ban-parking",
"ban-bug",
"ban",
"ballot",
"balloon",
"ball-pile",
"baht-sign",
"bahai",
"baguette",
"bags-shopping",
"bagel",
"badminton",
"badger-honey",
"badge-sheriff",
"badge-percent",
"badge-dollar",
"bacterium",
"bacteria",
"bacon",
"backward-step",
"backward-fast",
"backpack",
"baby-carriage",
"b",
"axe-battle",
"axe",
"award-simple",
"avocado",
"austral-sign",
"audio-description-slash",
"atom",
"at",
"asterisk",
"arrows-up-to-line",
"arrows-up-down-left-right",
"arrows-up-down",
"arrows-turn-to-dots",
"arrows-turn-right",
"arrows-to-line",
"arrows-to-eye",
"arrows-to-dotted-line",
"arrows-to-dot",
"arrows-to-circle",
"arrows-split-up-and-left",
"arrows-spin",
"arrows-rotate",
"arrows-retweet",
"arrows-repeat-1",
"arrows-repeat",
"arrows-minimize",
"arrows-maximize",
"arrows-left-right-to-line",
"arrows-left-right",
"arrows-from-line",
"arrows-from-dotted-line",
"arrows-down-to-people",
"arrows-down-to-line",
"arrows-cross",
"arrow-up-z-a",
"arrow-up-wide-short",
"arrow-up-triangle-square",
"arrow-up-to-line",
"arrow-up-to-dotted-line",
"arrow-up-square-triangle",
"arrow-up-small-big",
"arrow-up-short-wide",
"arrow-up-right-dots",
"arrow-up-right-and-arrow-down-left-from-center",
"arrow-up-right",
"arrow-up-long",
"arrow-up-left-from-circle",
"arrow-up-left",
"arrow-up-from-water-pump",
"arrow-up-from-square",
"arrow-up-from-line",
"arrow-up-from-ground-water",
"arrow-up-from-dotted-line",
"arrow-up-from-arc",
"arrow-up-big-small",
"arrow-up-arrow-down",
"arrow-up-a-z",
"arrow-up-9-1",
"arrow-up-1-9",
"arrow-turn-up",
"arrow-turn-down-right",
"arrow-turn-down-left",
"arrow-turn-down",
"arrow-trend-up",
"arrow-trend-down",
"arrow-rotate-right",
"arrow-rotate-left",
"arrow-right-to-line",
"arrow-right-to-city",
"arrow-right-to-bracket",
"arrow-right-to-arc",
"arrow-right-long-to-line",
"arrow-right-long",
"arrow-right-from-line",
"arrow-right-from-bracket",
"arrow-right-from-arc",
"arrow-right-arrow-left",
"arrow-pointer",
"arrow-left-to-line",
"arrow-left-long-to-line",
"arrow-left-long",
"arrow-left-from-line",
"arrow-down-z-a",
"arrow-down-wide-short",
"arrow-down-up-lock",
"arrow-down-up-across-line",
"arrow-down-triangle-square",
"arrow-down-to-square",
"arrow-down-to-dotted-line",
"arrow-down-to-bracket",
"arrow-down-to-arc",
"arrow-down-square-triangle",
"arrow-down-small-big",
"arrow-down-short-wide",
"arrow-down-right",
"arrow-down-long",
"arrow-down-left-and-arrow-up-right-to-center",
"arrow-down-left",
"arrow-down-from-line",
"arrow-down-from-dotted-line",
"arrow-down-big-small",
"arrow-down-arrow-up",
"arrow-down-a-z",
"arrow-down-9-1",
"arrow-down-1-9",
"archway",
"apple-whole",
"apple-core",
"apostrophe",
"apartment",
"ankh",
"angles-up",
"angles-right",
"angles-left",
"angles-down",
"angle-right",
"angle-left",
"angle-down",
"angle-90",
"angle",
"angel",
"anchor-lock",
"anchor-circle-xmark",
"anchor-circle-exclamation",
"anchor-circle-check",
"ampersand",
"amp-guitar",
"alt",
"align-right",
"align-left",
"align-justify",
"align-center",
"alien",
"alicorn",
"album-collection-circle-user",
"album-collection-circle-plus",
"album-collection",
"album-circle-user",
"album-circle-plus",
"alarm-plus",
"alarm-exclamation",
"airplay",
"air-conditioner",
"acorn",
"accent-grave",
"abacus",
"a",
"9",
"8",
"7",
"6",
"5",
"4",
"360-degrees",
"3",
"2",
"1",
"00",
"0",
]




brands = [
"facebook",
"twitter",
"instagram",
"tiktok",
"linkedin",
"github",
"discord",
"youtube",
"wordpress",
"slack",
"figma",
"apple",
"google",
"stripe",
"algolia",
"docker",
"windows",
"paypal",
"stack-overflow",
"kickstarter",
"dribbble",
"dropbox",
"squarespace",
"android",
"shopify",
"medium",
"codepen",
"cloudflare",
"airbnb",
"vimeo",
"whatsapp",
"intercom",
"usps",
"wix",
"line",
"behance",
"openid",
"product-hunt",
"internet-explorer",
"pagelines",
"teamspeak",
"html5",
"telegram",
"pinterest",
"dashcube",
"ideal",
"salesforce",
"readme",
"free-code-camp",
"soundcloud",
"square-twitter",
"accessible-icon",
"cc-visa",
"goodreads-g",
"google-play",
"react",
"wikipedia-w",
"square-js",
"java",
"square-pinterest",
"python",
"skype",
"linux",
"node",
"rebel",
"etsy",
"discourse",
"amazon",
"glide-g",
"gitlab",
"spotify",
"think-peaks",
"microsoft",
"elementor",
"pied-piper",
"square-youtube",
"cc-mastercard",
"facebook-messenger",
"atlassian",
"playstation",
"fly",
"meetup",
"twitch",
"waze",
"zhihu",
"yoast",
"yelp",
"yarn",
"yandex-international",
"yandex",
"yammer",
"yahoo",
"y-combinator",
"xing",
"xbox",
"wpressr",
"wpforms",
"wpexplorer",
"wpbeginner",
"wordpress-simple",
"wolf-pack-battalion",
"wodu",
"wizards-of-the-coast",
"wirsindhandwerk",
"whmcs",
"weixin",
"weibo",
"weebly",
"watchman-monitoring",
"vuejs",
"vnv",
"vk",
"vine",
"vimeo-v",
"viber",
"viadeo",
"viacoin",
"vaadin",
"ussunnah",
"usb",
"ups",
"untappd",
"unsplash",
"unity",
"uniregistry",
"uncharted",
"umbraco",
"uikit",
"ubuntu",
"uber",
"typo3",
"tumblr",
"trello",
"trade-federation",
"themeisle",
"themeco",
"the-red-yeti",
"tencent-weibo",
"symfony",
"swift",
"suse",
"supple",
"superpowers",
"stumbleupon-circle",
"stumbleupon",
"studiovinari",
"stubber",
"stripe-s",
"strava",
"sticker-mule",
"steam-symbol",
"steam",
"staylinked",
"stackpath",
"stack-exchange",
"square-xing",
"square-whatsapp",
"square-vimeo",
"square-viadeo",
"square-tumblr",
"square-steam",
"square-snapchat",
"square-reddit",
"square-pied-piper",
"square-odnoklassniki",
"square-lastfm",
"square-instagram",
"square-hacker-news",
"square-google-plus",
"square-gitlab",
"square-github",
"square-git",
"square-font-awesome-stroke",
"square-font-awesome",
"square-facebook",
"square-dribbble",
"square-behance",
"speaker-deck",
"speakap",
"space-awesome",
"sourcetree",
"snapchat",
"slideshare",
"skyatlas",
"sketch",
"sitrox",
"sith",
"sistrix",
"simplybuilt",
"shopware",
"shirtsinbulk",
"servicestack",
"sellsy",
"sellcast",
"searchengin",
"scribd",
"screenpal",
"schlix",
"sass",
"safari",
"rust",
"rockrms",
"rocketchat",
"rev",
"resolving",
"researchgate",
"replyd",
"renren",
"redhat",
"reddit-alien",
"reddit",
"red-river",
"reacteurope",
"ravelry",
"raspberry-pi",
"r-project",
"quora",
"quinscape",
"qq",
"pushed",
"pix",
"pinterest-p",
"pied-piper-pp",
"pied-piper-hat",
"pied-piper-alt",
"php",
"phoenix-squadron",
"phoenix-framework",
"phabricator",
"periscope",
"perbyte",
"patreon",
"palfed",
"page4",
"padlet",
"osi",
"orcid",
"optin-monster",
"opera",
"opencart",
"old-republic",
"odysee",
"odnoklassniki",
"octopus-deploy",
"nutritionix",
"ns8",
"npm",
"node-js",
"nimblr",
"nfc-symbol",
"nfc-directional",
"neos",
"napster",
"monero",
"modx",
"mizuni",
"mixer",
"mixcloud",
"mix",
"microblog",
"meta",
"mendeley",
"megaport",
"medrt",
"medapps",
"mdb",
"maxcdn",
"mastodon",
"markdown",
"mandalorian",
"mailchimp",
"magento",
"lyft",
"linode",
"linkedin-in",
"less",
"leanpub",
"lastfm",
"laravel",
"korvue",
"kickstarter-k",
"keycdn",
"keybase",
"kaggle",
"jsfiddle",
"js",
"joomla",
"joget",
"jira",
"jenkins",
"jedi-order",
"itunes-note",
"itunes",
"itch-io",
"ioxhost",
"invision",
"instalod",
"imdb",
"hubspot",
"houzz",
"hotjar",
"hornbill",
"hooli",
"hive",
"hire-a-helper",
"hips",
"hashnode",
"hackerrank",
"hacker-news",
"gulp",
"guilded",
"grunt",
"gripfire",
"grav",
"gratipay",
"google-wallet",
"google-plus-g",
"google-plus",
"google-pay",
"google-drive",
"goodreads",
"golang",
"gofore",
"glide",
"gitter",
"gitkraken",
"github-alt",
"git-alt",
"git",
"gg-circle",
"gg",
"get-pocket",
"galactic-senate",
"galactic-republic",
"fulcrum",
"freebsd",
"foursquare",
"forumbee",
"fort-awesome-alt",
"fort-awesome",
"fonticons-fi",
"fonticons",
"font-awesome",
"flipboard",
"flickr",
"firstdraft",
"first-order-alt",
"first-order",
"firefox-browser",
"firefox",
"fedora",
"fedex",
"fantasy-flight-games",
"facebook-f",
"expeditedssl",
"evernote",
"ethereum",
"erlang",
"envira",
"empire",
"ember",
"ello",
"edge-legacy",
"edge",
"ebay",
"earlybirds",
"dyalog",
"drupal",
"draft2digital",
"dochub",
"digital-ocean",
"digg",
"diaspora",
"dhl",
"deviantart",
"dev",
"deskpro",
"deploydog",
"delicious",
"deezer",
"dailymotion",
"d-and-d-beyond",
"d-and-d",
"cuttlefish",
"css3-alt",
"css3",
"critical-role",
"creative-commons-zero",
"creative-commons-share",
"creative-commons-sampling-plus",
"creative-commons-sampling",
"creative-commons-sa",
"creative-commons-remix",
"creative-commons-pd-alt",
"creative-commons-pd",
"creative-commons-nd",
"creative-commons-nc-jp",
"creative-commons-nc-eu",
"creative-commons-nc",
"creative-commons-by",
"creative-commons",
"cpanel",
"cotton-bureau",
"contao",
"connectdevelop",
"confluence",
"codiepie",
"cmplid",
"cloudversify",
"cloudsmith",
"cloudscale",
"chromecast",
"chrome",
"centos",
"centercode",
"cc-stripe",
"cc-paypal",
"cc-jcb",
"cc-discover",
"cc-diners-club",
"cc-apple-pay",
"cc-amex",
"cc-amazon-pay",
"canadian-maple-leaf",
"buysellads",
"buy-n-large",
"buromobelexperte",
"buffer",
"btc",
"bots",
"bootstrap",
"bluetooth-b",
"bluetooth",
"blogger-b",
"blogger",
"blackberry",
"black-tie",
"bity",
"bitcoin",
"bitbucket",
"bimobject",
"bilibili",
"battle-net",
"bandcamp",
"aws",
"aviato",
"avianex",
"autoprefixer",
"audible",
"asymmetrik",
"artstation",
"apple-pay",
"apper",
"app-store-ios",
"app-store",
"angular",
"angrycreative",
"angellist",
"amilia",
"amazon-pay",
"alipay",
"affiliatetheme",
"adversal",
"adn",
"accusoft",
"500px",
"42-group",
]

# raiz = ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/solid/", './../classic/solid/', ' classic solid']


# alarm-snooze classic solid.svg
# album classic solid.svg
# album-circle-plus classic solid.svg
# album-circle-user classic solid.svg
# album-collection classic solid.svg
# album-collection-circle-plus classic solid.svg
# album-collection-circle-user classic solid.svg
# alicorn classic solid.svg
# alien classic solid.svg
# alien-8bit classic solid.svg

raices = [
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/solid/", './../classic/solid/', 'classicSolid.mjs', ' classic solid', '__classic_solid', "normal"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/regular/", './../classic/regular/', 'classicRegular.mjs', ' classic regular', '__classic_regular', "normal"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/light/", './../classic/light/', 'classicLight.mjs', ' classic light', '__classic_light', "normal"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/thin/", './../classic/thin/', 'classicThin.mjs', ' classic thin', '__classic_thin', "normal"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/duotone/", './../classic/duotone/', 'classicDuotone.mjs', ' classic duotone', '__classic_duotone', "duotone"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/sharp-solid/", './../sharp/solid/', 'sharpSolid.mjs', ' sharp solid', '__sharp_solid', "normal"],
    ["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/sharp-regular/", './../sharp/regular/', 'sharpRegular.mjs', ' sharp regular', '__sharp_regular', "normal"],
]

# export var mdiArrangeBringForward = "M2,2H16V16H2V2M22,8V22H8V18H10V20H20V10H18V8H22Z";

# jsFile = open('jsfile.js', 'w')

def descargar(array, limite):
    jsFile = open(array[2], 'w')

    with os.scandir(array[1]) as ficheros:
        for fichero in ficheros:
            # print(fichero.name)

            file_name = array[1] + fichero.name

            file = open(file_name, 'r')

            # texto = re.search('(?<=<path d=)["\w\. -]+(?=/>)', file.read())
            texto = file.read()



            nombre = re.search("^[\w-]+(?="+limite+")", fichero.name).group()

            nombre = nombre.replace('-', '_')

            paths = re.findall('(?<=d=")[\w .,-]+(?=")', texto)
            # path = re.search('(?<=<path d=")[\w .,-]+(?="/?>)', texto)
            # if paths:
            #     print("\t", paths)
            # else:
            #     print("\tPath no encontrado en: ", texto)

            variables_start = "{"
            variable_vb = "vb: '"+re.search('(?<=viewBox=")\d \d \d+ \d+(?=")', texto).group()+"',"
            variable_tipo = "type: '"+array[5]+"',"

            try:
                paths_start = \
                    "paths: {" \
                        "primary: {" \
                            "path: '"+re.search('(?<=class="fa-primary" d=")[\w .,-]+(?=")', texto).group()+"'"\
                        "}," \
                        "secondary: {" \
                            "path: '"+re.search('(?<=class="fa-secondary" d=")[\w .,-]+(?=")', texto).group()+"'"\
                        "},"\
                    "},"
            except:
                paths_start = \
                    "paths: {" \
                        "primary: {" \
                            "path: '" + re.search('(?<=d=")[\w .,-]+(?=")', texto).group() + "',"\
                        "}," \
                    "},"


            variables_end = "}"

            jsFile.write(f"export const jvi_{nombre}{array[4]} = {variables_start}{variable_vb}{variable_tipo}{paths_start}{variables_end};\n")
            # jsFile.write(f"export const jvi_{nombre}{array[4]} = '{texto}';\n")
            # jsFile.write(f"export const jvi_{nombre}{array[4]} = 'data:image/svg+xml;charset=utf-8,{texto}';\n")

    jsFile.close()
    print('terminado: '+array[2])

    # for nombre in nombres:
    #     file_name = array[1] + nombre + array[2] + '.svg'
    #
    #     file = open(file_name, 'r')
    #
    #     texto = re.search('', file.read())
    #
    #     jsFile.write(f'export let \n')
    #
    #     data = requests.get(array[0] + nombre + ".svg")
    #
    #     file.close()

        # res_status = str(data.status_code)
        #
        # if res_status[:2] == "20":
        #     fileName = array[1] + nombre + array[2] + '.svg'
        #     with open(fileName, 'wb') as file:
        #         file.write(data.content)
        #         print('\033[92m  ' + res_status + '  |  Archivo: "' + fileName + '"\033[0m')
        #         file.close()
        # else:
        #     print('\033[91m  ' + res_status + '  |  Archivo: "' + fileName + '"\033[0m')
        #
        # pass


if __name__ == '__main__':
    descargar(["https://site-assets.fontawesome.com/releases/v6.3.0/svgs/brands/", './../brands/', 'brands.mjs', '', '__brands', "normal"], ".svg")
    print("terminado: brands")

    for i in range(len(raices)):
        descargar(raices[i], " ")




    # jsFile.close()

    print('La ejecución ha terminado')